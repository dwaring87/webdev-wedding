          ...
          <div class="flex flex-wrap md:flex-nowrap gap-4 items-center">
            <div class="max-w-full lg:max-w-sm">
              ...
              <p v-if="rec.website && rec.website.length > 0">
                ...
                <span class="inline-block mr-4" v-for="(w) in rec.website"><a :href="w.url" target="_blank">{{ w.title }}</a></span>
              </p>
              <p v-if="rec.map && rec.map.length > 0">
                <strong>Map:</strong>&nbsp;
                <span class="inline-block mr-4" v-for="(m) in rec.map"><a :href="m.url" target="_blank">{{ m.title }}</a></span>
              </p>
              <br />
              <p> {{ rec.description }}</p>
            </div>
